...
### Zeitlicherverlauf für `r renderText(get_landkreis_by_id(active_landkreis()))`
```{r}
...
output$table_of_landkreis <-renderDT(
    {
      data_landkreise_per_day() %>%
        filter(IdLandkreis == active_landkreis()) %>%
        ungroup() %>% 
        select(Meldedatum, infected, infected_7_per_100k, delta_7_per_100k) %>% 
        mutate(
          R = round(infected_7_per_100k / (infected_7_per_100k - delta_7_per_100k), 2)
        ) %>% 
...
### `r renderText({ paste0(active_bundesland(), " am ", format(date(), format = "%d.%m.%Y"))  })`
